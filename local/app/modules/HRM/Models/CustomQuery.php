...
//        return DB::getQueryLog();
    ...
    public static function getUserInformation($limit, $offset)
    {
        $users = DB::connection('hrm')->table('tbl_user')
            ...
            ->join('tbl_user_log', 'tbl_user_log.user_id', '=', 'tbl_user.id')->orderBy('tbl_user.user_name')->skip($offset)->take($limit)
            ->select('tbl_user.id','tbl_user.status', 'tbl_user.user_name', 'tbl_user_details.first_name', 'tbl_user_details.last_name', 'tbl_user_details.email', 'tbl_user_log.last_login', 'tbl_user_log.user_status', 'tbl_user.status','tbl_logged_in_user.id as logged_in')->orderBy('logged_in','desc')
            ->get();
        return $users;
    }
...
// Dashboard panel ansar list
    public static function getTotalPaneledAnsarList($offset, $limit, $unit, $thana, $division = null, $time, $rank, $q)
    {
        ...
        $total = clone $ansarQuery;
        $total->groupBy('tbl_designations.id')->select(DB::raw("count('tbl_ansar_parsonal_info.ansar_id') as t"), 'tbl_designations.code');
        $ansars = $ansarQuery->orderBy('tbl_panel_info.panel_date')->select('tbl_ansar_parsonal_info.ansar_id as id', 'tbl_ansar_parsonal_info.ansar_name_bng as name', 'tbl_ansar_parsonal_info.data_of_birth as birth_date',
...
// Dashboard embodied ansar list
    public static function getTotalEmbodiedAnsarList($offset, $limit, $unit, $thana, $division = null, $time, $rank, $q)
    {
        ...
        $total->groupBy('tbl_designations.id')->select(DB::raw("count('tbl_ansar_parsonal_info.ansar_id') as t"), 'tbl_designations.code');
        $ansars = $ansarQuery->distinct()->select('tbl_ansar_parsonal_info.ansar_id as id', 'tbl_ansar_parsonal_info.ansar_name_bng as name', 'tbl_ansar_parsonal_info.data_of_birth as birth_date',
            'tbl_designations.name_bng as rank', 'tbl_units.unit_name_bng as unit', 'tbl_thana.thana_name_bng as thana', 'tbl_kpi_info.kpi_name', 'tbl_embodiment.joining_date', 'tbl_embodiment.memorandum_id')->orderBy('tbl_embodiment.joining_date','desc')->skip($offset)->limit($limit)->get();
...
//        print_r(DB::getQueryLog());
    ...
    public static function threeYearsOverAnsarList($offset, $limit, $division, $unit, $ansar_rank, $ansar_sex)
    {
        ...
        $ansars = $ansarQuery->select('tbl_embodiment.ansar_id as id', 'tbl_embodiment.reporting_date as r_date', 'tbl_embodiment.joining_date as j_date', 'tbl_designations.id as did',
             'tbl_kpi_info.kpi_name as kpi', 'tbl_ansar_parsonal_info.ansar_name_bng as name', 'tbl_ansar_parsonal_info.data_of_birth as birth_date', 'tbl_units.unit_name_bng as unit', 'tbl_designations.name_bng as rank')->skip($offset)->limit($limit)->get();
